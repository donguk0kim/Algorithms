...
void IntegerSet::insert(int val) {
  ...
  int internal = bucket_index(val, numBuckets);
...
void IntegerSet::remove(int val) {
  Bucket* bucket = &store[bucket_index(val, numBuckets)];
  bucket->remove(val);
  count--;
}
...
int IntegerSet::bucket_index(int val, int nBuckets) const {
...
void IntegerSet::resize() {
  ...
  for (int indexRow = 0; indexRow < numBuckets; indexRow++) {
    ...
    for (int indexCol = 0; indexCol < bucket->length(); indexCol++) {
      ...
      bucketIdx = bucket_index(value, new_buckets);
